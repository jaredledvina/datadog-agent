...
func init() {
	...
	checkCmd.Flags().StringVar(&profileMemoryDepth, "m-depth", "", "the number of stack frames to consider")
	...
	checkCmd.Flags().MarkHidden("m-depth")
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if profileMemory {
			...
			for idx := range allConfigs {
				...
				data["profile_memory"] = filepath.Join(profileMemoryDir, checkName)
		...
		for _, c := range cs {
			...
			if formatJSON {
				...
				// Every instance will create its own directory, though only the unique
				// part is used since colons can't be part of a file path on Windows
				instanceID := strings.Split(string(c.ID()), ":")[1]
...
func populateMemoryProfileConfig(initConfig map[string]interface{}) error {
	if profileMemoryDepth != "" {
		profileMemoryDepth, err := strconv.Atoi(profileMemoryDepth)
		if err != nil {
			return fmt.Errorf("--m-depth must be an integer")
		}
		initConfig["profile_memory_depth"] = profileMemoryDepth
	}
